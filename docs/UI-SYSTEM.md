...
- Selected-day workspace current runtime’da root takvimin hemen altında inline/fold üstü okunur; mobile ve desktop hiyerarşisi `month root -> selected day workspace -> detail/create-edit katmanları` sırasıyla net kalır. Mobile/tablet’te gün seçimi sonrası workspace sert zıplama olmadan kontrollü küçük bir aşağı kaymayla viewport’a daha görünür gelir.
...
- Staff işlem formunda müşteri alanı current runtime’da tek picker trigger olarak görünür; create ve edit yüzeyinde ayrı `Mevcut müşteri / Yeni müşteri` sekmeleri veya duplicate üst müşteri özeti taşınmaz. Seçili durumda görünür ana bilgi yalnız müşteri adıdır.
- Staff müşteri picker aynı katmanda arama + create yan akışını birlikte taşır. Arama inputu `Ad soyad, telefon veya e-posta ile ara` davranışıyla çalışır; sonuçlar tek listede görünür, seçim form state’ini doğrudan günceller.
- Picker içi `Yeni müşteri` akışı aynı dialog/sheet içinde kalır; varsayılan görünür alanlar `Ad soyad` ve `Telefon`dur, `E-posta` disclosure ile opsiyonel açılır. Başarıda yeni müşteri otomatik seçilir, picker kapanır ve mevcut tarih/saat/işlem tipi/tutar/not bağlamı korunur. Redirect veya ham teknik hata stringi görünmez.
- Staff detail sheet read-first davranır; müşteri + tarih/saat bloğunun hemen altında read-only `İşlem özeti` görünür. Bu özet hızlı bakış ritminde `İşlem tipi`, `Toplam` ve `Kapora` alanlarını taşır; artist seçimi edit formunda kalır ve summary mini rapor yoğunluğuna çıkmaz.
...
- Staff edit sheet form-first kalır; üstte yalnız kısa müşteri bağlamı görünür ve duplicate read-only `İşlem özeti` edit üstünden kaldırılmıştır. Appointment ve walk-in aynı form omurgasını reuse eder; inputlar daha erken görünür kalır.
- Appointment ve walk-in detail sheet aynı summary kontratını taşır; `İşlem özeti` quick preview katmanında kalır ve alan adları customer detail `İşlem özeti` kartıyla uyumlu kalır.
