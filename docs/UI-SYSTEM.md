...
- Kasa: current runtime’da gün özeti ve defter service-intake kaynaklı otomatik hareketleri taşır; manuel form yalnız istisna / gider / correction yüzeyi olarak kalır. `Gelir / Gider` kontrolü, kısa kategori etiketi, tutar ve `Kaydı ekle` akışı manuel kayıt için yaşar; sistem satırları read-only kalır. Yüzey cash-only çalışır; ödeme tipi görünmez. Mobile öncelik sırası `Bugün özeti -> Defter -> Manuel giriş` olarak okunur; desktop’ta manuel form secondary sağ kolon gibi kalır. Gün özeti ana referans, defter ve tarih seçici seçili gün kontrolü için çalışır. Header sağ aksiyonu current runtime’da `Manuel giriş` sayfa içi atlamasıdır. Disclosure row mobile-safe kalır ve x-overflow üretmez.
