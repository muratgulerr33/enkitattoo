...
- Current runtime lock PR-C ile staff tarafında appointment-first month root + unified day workspace akışına genişlemiştir: appointment create/edit/delete, walk-in create/edit, appointment-linked `service_intakes` foundation, service-intake collection event’lerinden otomatik beslenen cash ledger ve combined consent runtime’ı repo içinde doğrulanır.
- Planned roadmap yönü unified service session / intake omurgasının belge ve rollout katmanlarıdır. PR-D document packet + print + 2-signature, PR-E final polish / rollout / migration cleanup hedefidir.
