...
- Current runtime lock PR-B ile staff tarafında appointment-first month root + unified day workspace akışına genişlemiştir: appointment create/edit/delete, walk-in create/edit, appointment-linked `service_intakes` foundation, combined consent runtime’ı ve manuel cash ledger repo içinde doğrulanır.
- Planned roadmap yönü unified service session / intake omurgasıdır. PR-B appointment + walk-in için ortak workspace, PR-C cashbook automation, PR-D document packet + print + 2-signature, PR-E final polish / rollout / migration cleanup hedefidir.
- `service_intakes` şeması `flowType = walk_in` enum değerini foundation olarak taşır. Current runtime’da yeni route açmadan `/ops/staff/randevular` selected-day workspace’i içinde walk-in create/edit akışı kanıtlıdır; month root ve occupancy ise appointment-first kalır. `service session` ifadesi ayrı modül adı değil, bu ortak işlem omurgasının ürün dilidir.
...
- Staff customer detail tek combined sözleşme onayını `workspace.latestConsent` ve `hasCurrentConsent` üstünden sürüm + onay tarihi ile özetler (`src/lib/ops/user-workspace.ts`, `src/app/ops/staff/musteriler/[userId]/page.tsx`).
...
- Staff appointments V2 görünür akışı month-first root -> day sheet -> detail sheet -> create/edit sheet zinciridir; month cell içinde exact count yerine decoration tabanlı occupancy kullanılır. Current runtime’da selected day sheet appointment + walk-in kayıtlarını aynı liste içinde source-aware olarak gösterir, fakat month root occupancy appointment-first kalır. UI ritmi `docs/UI-SYSTEM.md` içinde yaşar (`src/app/ops/staff/randevular/page.tsx`, `src/components/ops/ops-staff-appointments-workspace.tsx`).
- Staff create/edit formu current runtime’da browser native required tooltip’ine yaslanmaz; `customerUserId`, `sessionSource`, tarih/saat ve tutar hataları typed action state üzerinden kısa app-level mesaj olarak döner (`src/components/ops/ops-staff-appointment-create-form.tsx`, `src/app/ops/randevular/actions.ts`).
...
- Staff walk-in create current runtime’da typed state action ile aynı workspace içinde kalır; yeni kayıt `flowType = walk_in`, `appointmentId = null` ile oluşur ve success copy `Walk-in işlem kaydı açıldı.` olur (`src/app/ops/randevular/actions.ts`, `src/lib/ops/service-intakes.ts`).
- Staff appointments detail surface’i bağlı `service_intake` özetini doğrudan işlem bağlamında gösterir. Appointment summary verisi `/ops/staff/randevular` server page içinde `listLatestServiceIntakesByAppointmentIds()` ile, walk-in kayıtları ise `listWalkInServiceIntakesForMonth()` ile toplanır; unified day/detail/edit workspace bu iki kaynağı source-aware olarak birleştirir (`src/app/ops/staff/randevular/page.tsx`, `src/lib/ops/service-intakes.ts`, `src/components/ops/ops-staff-appointments-workspace.tsx`).
...
- Walk-in aynı gün ve aynı saatte birden fazla olabilir; current runtime walk-in create/edit zinciri için slot conflict guard veya uyarı yazmaz (`src/app/ops/randevular/actions.ts`, `src/lib/ops/service-intakes.ts`).
...
- Staff kasa yüzeyi hızlı kayıt + defter mantığıyla çalışır; ana akış yön seçimi, kısa kategori etiketi, tutar ve kayıt submit zinciridir. Current runtime kullanıcı yüzeyinde cash-only davranır; ödeme tipi görünmez ve create/update action’ları yeni kayıtları `cash` olarak yazar (`src/app/ops/staff/kasa/page.tsx`, `src/components/ops/ops-cash-entry-form.tsx`, `src/app/ops/kasa/actions.ts`).
- Cashbook automation current runtime değildir; mevcut kasa davranışı manuel entry + summary + history manage omurgasıyla sınırlıdır. Appointment veya gelecekteki service-session kayıtlarından otomatik posting yapılmaz (`src/app/ops/staff/kasa/page.tsx`, `src/lib/ops/cashbook.ts`).
...
- Rapor hesaplamaları mevcut `cash_entries` ve `appointments` omurgasını reuse eder; yeni analytics schema veya chart sistemi açmaz (`src/lib/ops/reports.ts`, `src/lib/ops/cashbook.ts`, `src/lib/ops/appointments.ts`).
