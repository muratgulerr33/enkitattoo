...
- Ops-visible para display current runtime’da kuruşsuz whole-number ritmine hizalanmıştır; shared kasa/rapor helper’ı ile müşteri detail, staff işlem summary ve belge preview aynı sade sayı gösterimini kullanır. `formatCashAmountInput()` ve benzeri input serialization helper’ları bu display kontratına dahil değildir (`src/lib/ops/cashbook.ts`, `src/lib/ops/money.ts`, `src/app/ops/staff/musteriler/[userId]/page.tsx`, `src/components/ops/ops-staff-appointments-workspace.tsx`, `src/components/ops/ops-staff-document-packet.tsx`).
