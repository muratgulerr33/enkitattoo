...
- Selected-day workspace current runtime’da root takvimin hemen altında inline/fold üstü konumlanır; mobile/tablet’te gün seçimi sonrası controlled küçük bir reveal scroll ile workspace viewport’ta daha erken görünür, desktopta bu davranış çalışmaz (`src/app/ops/staff/randevular/page.tsx`, `src/components/ops/ops-staff-appointments-workspace.tsx`).
...
- Staff appointments edit surface current runtime’da duplicate summary taşımaz; edit sheet üstünde yalnız kısa müşteri/tarih/saat bağlamı kalır ve `İşlem tipi`, `Toplam`, `Kapora` değerleri form alanları içinden düzenlenir (`src/components/ops/ops-staff-appointments-workspace.tsx`, `src/components/ops/ops-staff-appointment-create-form.tsx`).
