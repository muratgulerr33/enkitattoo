...
- `/ops/user/form` route’u user-facing tarafta `dövme detayları` diliyle sunulur; profil ve randevular CTA’larıyla aynı semantik aileyi paylaşır. Teknik snapshot omurgası içeride yaşamaya devam eder; route kaldırılmamış, yeniden adlandırılmamış veya nav’a geri alınmamıştır (`src/app/ops/user/form/page.tsx`, `src/components/ops/ops-tattoo-form.tsx`, `src/app/ops/user/profil/page.tsx`, `src/app/ops/user/randevular/page.tsx`, `src/lib/ops/user-workspace.ts`).
