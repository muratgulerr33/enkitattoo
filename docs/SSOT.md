...
- Current runtime lock PR-A foundation ile sınırlıdır: staff tarafında appointment-first create/edit/delete akışı, appointment-linked `service_intakes` foundation, combined consent runtime’ı ve manuel cash ledger repo içinde doğrulanır.
...
- `service_intakes` şeması `flowType = walk_in` enum değerini foundation olarak taşır; ancak current runtime’da kanıtlı walk-in route, action veya workspace yoktur. `service session` ifadesi bu aşamada ürün yönüdür, current runtime modül adı değildir.
...
- Current runtime’da `service_intakes` için tek kanıtlı write/read omurgası staff randevu V2 zinciridir; walk-in intake workspace henüz route seviyesinde açılmamıştır.
